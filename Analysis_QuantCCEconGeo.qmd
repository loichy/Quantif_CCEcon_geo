...
```{r}
model3<- lm(N_CC ~ country + PY + PIB_billion + RDValues + EducFValues + EducHValues + CO2_mega + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) 
...
```{r}
model1<- lm(N_ratio ~ PIB_billion + RDValues + EducFValues + EducHValues + CO2emissionsalues + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) # modèle sans effet fixe
vif(mod=model1)
stargazer(model1, 
          type = "text",
          keep=c("PIB_billion","RDValues","EducFValues","EducHValues","CO2emissionsalues","NDC_Implied_average", "is_carbon_tax","ExpendituresValues","DisastersValues","TemperatureValues","Comparative_advantage"))
