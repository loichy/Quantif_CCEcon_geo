...
selected_data <- df_final_merged[,c("N_CC","N_ratio","PIBValues","RDValues","EducFValues","EducHValues","CO2emissionsalues","NDC_Implied_average","NDC_Implied_conditional","NDC_Implied_unconditional","is_carbon_tax","ExpendituresValues","DisastersValues","TemperatureValues")]
...
Analyse de corrélation PIB à l'échelle désagrégée
...
#log?
ggplot(df_aggreg_countries_PIB, aes(x = mean_PIB_country, y = N_CC_country)) +
  geom_point() +
...
```{r}
ggplot(df_aggreg_countries_PIB, aes(x = log(mean_PIB_country), y = log(N_ratio))) +
  geom_point() +
  geom_smooth(method = "lm", col = "blue") +
  #geom_text(data = filter(df_aggreg_countries_PIB), aes(x=mean_PIB_country), y= N_CC_country, label=country, nudge_x =1, nudge_y=1)+
...
```{r}
model1<- lm(N_CC ~ PIBValues + RDValues + EducFValues + EducHValues + CO2emissionsalues + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) # modèle sans effet fixe
...
```{r}
model2<- lm(N_CC ~ country + PIBValues + RDValues + EducFValues + EducHValues + CO2emissionsalues + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) 
...
```{r}
model3<- lm(N_CC ~ country + PY + PIBValues + RDValues + EducFValues + EducHValues + CO2emissionsalues + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) 
...
```{r}
model1<- lm(N_ratio ~ PIBValues + RDValues + EducFValues + EducHValues + CO2emissionsalues + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) # modèle sans effet fixe
...
```{r}
model2<- lm(N_ratio ~ country + PIBValues + RDValues + EducFValues + EducHValues + CO2emissionsalues + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) 
...
```{r}
model3<- lm(N_ratio ~ country + PY + PIBValues + RDValues + EducFValues + EducHValues + CO2emissionsalues + NDC_Implied_average + is_carbon_tax + ExpendituresValues + DisastersValues + TemperatureValues, data = df_final_merged) 
